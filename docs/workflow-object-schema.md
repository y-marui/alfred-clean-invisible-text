...
[`alfred-workflow-object-types.plist`](alfred-workflow-object-types.plist)
in this directory is the result of exactly that process (55 object types,
one of each, disconnected from each other — `connections` is empty). It's
the trust anchor for this document: if a config key's meaning here turns
out to be wrong, that file has the exact key names and default value types
Alfred itself wrote, which out-ranks any prose description below.
...
Everything below is a one-line best-effort label based on Alfred's own UI naming, generated by inspecting each type's config keys in
[`alfred-workflow-object-types.plist`](alfred-workflow-object-types.plist) — not independently verified by using each one. Treat the label as a starting guess and the linked plist as the actual source of truth for field names; a name in italics means lower confidence.

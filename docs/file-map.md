...
| File | Role | Key Dependencies |
|---|---|---|
| `workflow/info.plist` | The Alfred object graph: a self-triggered Keyword Script Filter (`cit`, clipboard) and a connection-only Script Filter (Universal Action target, not yet connected — see README.md Setup), both feeding a shared `run` Script Filter, feeding a `copy-report` Run Script | `cmd/clean-invisible-text-alfred` (invoked via relative path — Alfred sets CWD to the bundle root) |
...
Manually verified by building the package and running the extracted binary
from its own bundle directory (matching Alfred's actual invocation model,
including a real pinned-CLI `check` finding a ZWSP) — not yet verified
inside Alfred's own Workflow debugger, and not yet tested on real Intel
hardware (built here on Apple Silicon; the binary itself is a verified

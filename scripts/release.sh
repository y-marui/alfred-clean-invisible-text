...
#
# Unlike the Actions release, this does NOT attest build provenance —
# actions/attest-build-provenance needs the runner's OIDC token, which isn't
# available locally. `gh attestation verify` on a locally-built artifact will
# report no attestations found; that's expected.
#
# Usage:
...
echo "→ Fetching pinned CLI binaries"
make fetch-cli
...
echo "(no build provenance attestation — see script header)"
echo
